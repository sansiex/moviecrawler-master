...
function meizzToday()    //Today Button
{
    ...
    if (outObject) {
        if (bUseTime) {
            outObject.value = parent.meizzTheYear + dateSeperator + format(parent.meizzTheMonth) + dateSeperator +
...
function WriteDateTo(yy, mm, n, hh, mi, se) {
    if (bUseTime) {
        outObject.value = yy + dateSeperator + format(mm) + dateSeperator + format(n) + ' ' + format(hh) + ':' + format(mi) + ':' + format(se); //注：在这里你可以输出改成你想要的格式
    }
    else {
        outObject.value = yy + dateSeperator + format(mm) + dateSeperator + format(n); //注：在这里你可以输出改成你想要的格式
    }
}
